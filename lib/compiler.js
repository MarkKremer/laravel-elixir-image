...
function compilerHelper(_this, file, enc, cb, destPaths, i) {
    ...
    if(i < destPaths.length) {
        ...
        if(!config.convertSameType && sourceExt === destExt) {
            ...
            var proc = spawn('imagemagick/convert', [sourceExt + ':-', destExt + ':-']);
...
function compiler(options) {
    return through.obj(function(file, enc, cb) {
        ...
        // TODO: Find out why it doesn't work when I set a context using call()/apply() instead of giving it as an
        //       argument. Suspecting something with the garbage collector. Probably not a good idea to use those
        //       functions anyway.
